...
const StoreLayout = ({ children }) => {
  ...
  const fetchStoreAndListen = async () => {
    // ¡ATENCIÓN! Agregamos 'status' al select para poder proteger el menú
        ...
        <div className="px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-3">
              {/* CAMPANITA INTELIGENTE (Solo visible si la tienda está aprobada) */}
              {storeInfo?.status === 'active' && (
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside className="hidden lg:flex w-64 bg-white border-r border-gray-200 flex-col">
          <nav className="p-4 space-y-2 flex-1">
            {/* EL CANDADO: Solo dibuja el menú si el status es active */}
            {storeInfo?.status === 'active' && navItems.map((item) => {
        ...
        {sidebarOpen && (
          <>
            ...
            <aside className="lg:hidden fixed left-0 top-16 bottom-0 w-64 bg-white z-50 overflow-y-auto flex flex-col">
              <nav className="p-4 space-y-2 flex-1">
                {/* EL MISMO CANDADO PARA EL MÓVIL */}
                {storeInfo?.status === 'active' && navItems.map((item) => {
                  const Icon = item.icon;
                  return (
